...
# initilizes the SSH connections so we pass in the required parameters
# here
...
with open("port_maps.csv", "a") as csvfile:
    ...
    for switch in tor_switches:
        ...
        # Convieniently enough the keys of arp table parsing are the IP addresses
        ...
        for arp_key in arp_table[tables.router_hostname].keys():
            ...
            for mac_key in mac_table[tables.switch_hostname].keys():
                ...
                # If the mac address from the arp table is found in the mac address table and the interface is not a trunk
                # port, we have found a match that needs to be captured
